...
export default class ClienteCtrl {
  gravar(requisicao, resposta) {
    ...
    if (requisicao.method === "POST" && requisicao.is("application/json")) {
      ...
      if (nome && cpf && telefone) {
        const cliente = new Cliente(0, nome, cpf, telefone);
        cliente
          .gravar()
          .then(() => {
  ...
  atualizar(requisicao, resposta) {
    ...
    if (
      ...
      if (codigoC && nome && cpf && telefone) {
        const cliente = new Cliente(codigoC, nome, cpf, telefone);
        cliente
          .atualizar()
          .then(() => {
            resposta.status(200).json({
      ...
      resposta.status(400).json({
        status: false,
        mensagem:
          "Por favor, utilize os métodos PUT ou PATCH para atualizar o cliente!",
      });
    }
  }
